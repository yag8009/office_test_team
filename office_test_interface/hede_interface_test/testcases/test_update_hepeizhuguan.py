...
# @Author  : yag8009
# @FileName  : test_update_hepeizhuguan
# @Time    : 2020/4/21
import configparser
import unittest
...
from testcores.http_update_hepeizhuguan import http_update_hepeizhuguan

cf = configparser.ConfigParser()
cf.read("api_setting.ini")
file_path = cf.get("HepeizhuguanFilePath", "file_path")

class test_update_hepeizhuguan(unittest.TestCase):
    # 测试前预设
    def setUp(self):
        logger.info(" --------- 测试开始 --------- ")
    ...
    def action_hede(self, case_id, case_name,login_url, login_data, identity, base_url, base_date, from_url, from_data, assertion):
        ...
        action_page = http_update_hepeizhuguan(login_url, eval(login_data), identity, base_url, eval(base_date), from_url, eval(from_data))
...
def __TestCases():
    ...
    for args in range(1, table.nrows):
        ...
        setattr(test_update_hepeizhuguan, 'test_hede_%s' % txt[1], test_update_hepeizhuguan.getTestFunc(*txt))
