...
# @Author  : yag8009
# @FileName  : http_update_weixiu
# @Time    : 2020/4/21
import sys
import requests
from public.log import logger

def http_update_weixiu(login_url, login_data, identity, base_url, base_data, from_url, from_data):
    s = requests.session()
    ...
    if identity in req_login.text:
        ...
        req_base = s.post(url=base_url, data=base_data.encode('utf-8'), headers=header, timeout=(5, 10))
