...
from testcores.hede_interface_API import hede_test
...
class ActionHeDeTest(unittest.TestCase):
    ...
    def action_login(self, case_id, case_name, url, data, assertion, *arge):
        ...
        action_page = hede_test(url, eval(data), self.key)
        logger.info(u"返回数据:  " + action_page.text)
        self.assertEqual(action_page.json()["status"], assertion, action_page.text)
...
def __TestCases():
    cf = configparser.ConfigParser()
    cf.read("api_setting.ini")
    data = xlrd.open_workbook(cf.get("HeDeTestFilePath", "file_path"))
    # 通过索引顺序获取Excel表
    table = data.sheets()[0]
